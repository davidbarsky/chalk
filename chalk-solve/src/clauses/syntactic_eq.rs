...
use chalk_ir::{
    cast::Cast,
    fold::{shift::Shift, FallibleTypeFolder, TypeFoldable, TypeSuperFoldable},
    interner::Interner,
...
pub fn syn_eq_lower<I: Interner, T: TypeFoldable<I>>(
    interner: I,
    clause: T,
) -> Result<T, chalk_ir::NoSolution> {
    ...
    clause.try_fold_with(&mut folder, DebruijnIndex::INNERMOST)
}

struct SynEqFolder<I: Interner> {
    interner: I,
...
impl<I: Interner> FallibleTypeFolder<I> for SynEqFolder<I> {
    type Error = chalk_ir::NoSolution;

    fn as_dyn(&mut self) -> &mut dyn FallibleTypeFolder<I, Error = Self::Error> {
        self
    }

    fn try_fold_ty(&mut self, ty: Ty<I>, outer_binder: DebruijnIndex) -> Fallible<Ty<I>> {
        ...
        match ty.kind(interner) {
            ...
            _ => ty.try_super_fold_with(self, outer_binder),
        }
    }
    ...
    fn try_fold_program_clause(
        ...
        let ProgramClauseData(binders) = clause.data(interner.clone());

        let implication = binders.skip_binders();
        let mut binders: Vec<_> = binders.binders.as_slice(interner.clone()).into();
        ...
        let mut conditions = implication
            .conditions
            .clone()
            .try_fold_with(self, outer_binder)?;
    ...
    fn try_fold_goal(&mut self, goal: Goal<I>, outer_binder: DebruijnIndex) -> Fallible<Goal<I>> {
        assert!(self.new_params.is_empty());
        assert!(self.new_goals.is_empty());
        ...
        match goal.data(interner) {
            ...
            _ => return goal.try_super_fold_with(self, outer_binder),
        };
