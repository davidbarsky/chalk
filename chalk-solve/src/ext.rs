...
use crate::infer::{self, InferenceTable};
...
impl<I: Interner> GoalExt<I> for Goal<I> {
    ...
    fn into_peeled_goal(self, interner: I) -> UCanonical<InEnvironment<Goal<I>>> {
        ...
        let value = syn_eq_lower(interner, peeled_goal).unwrap();
        let canonical = infer.canonicalize(interner, value).quantified;
        infer::InferenceTable::<I>::u_canonicalize(interner, &canonical).quantified
    }
    ...
    fn into_closed_goal(self, interner: I) -> UCanonical<InEnvironment<Goal<I>>> {
        let mut infer = InferenceTable::new();
        let lowered_goal = syn_eq_lower(interner, self).unwrap();
